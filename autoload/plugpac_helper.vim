...
export def SetupCommands()
  ...
  command! -nargs=1 -complete=custom,PackComplete PackUrl   PackUrl(<q-args>)
  command! -nargs=1 -complete=custom,PackComplete PackDir   PackDir(<q-args>)
  command! -nargs=1 -complete=custom,PackComplete PackRc    PackRc(<q-args>)
  command! -nargs=1 -complete=custom,PackComplete PackRcPre PackRcPre(<q-args>)
  command! -bang PackUnusedRC cexpr PackUnusedRC(<q-bang> == '!')->map((_, x) => $"{x}:1:1: Unused")<BAR>copen
  command! -bar -nargs=1 -complete=custom,StartPluginComplete PackOpt call DisableEnablePlugin(<q-args>, true)
  command! -bar -nargs=1 -complete=custom,OptPluginComplete PackStart call DisableEnablePlugin(<q-args>, false)
enddef
